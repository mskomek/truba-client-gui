...
DEFAULT_LOCAL_AGENT_PATH = ROOT / "runner" / "local_agent.py"
DEFAULT_OLLAMA_COMMAND = os.environ.get("TRUBA_OLLAMA_COMMAND", "ollama")
DEFAULT_BUILDER_MODEL = os.environ.get("TRUBA_BUILDER_MODEL", "qwen3.5:4b")
DEFAULT_TESTER_MODEL = os.environ.get("TRUBA_TESTER_MODEL", "qwen3.5:9b")
...
LOCAL_VERIFY_PROMPT = "Output exactly: LOCAL_OLLAMA_OK"
...
def _run_ollama_diagnostic_command(
    command: list[str],
    timeout_seconds: int,
    *,
    ...
    try:
        ...
        return (
            {
                "command": evidence_command or command,
...
def _ollama_listed_models(output: str) -> set[str]:
    models: set[str] = set()
    for index, line in enumerate(output.splitlines()):
        columns = line.split()
        if not columns:
            continue
        if index == 0 and columns[0].upper() == "NAME":
            continue
        models.add(columns[0])
    return models


def verify_local_ollama(
    ollama_command: str,
    model: str,
    timeout_seconds: int,
    *,
    ...
    """Run explicit CLI/server/model checks and one tiny, opt-in local inference."""
    resolved_command = resolve_ollama_command(ollama_command)
    started_at = utc_timestamp()
    overall_start = time.monotonic()

    cli_check, _version_output = _run_ollama_diagnostic_command(
        [resolved_command, "--version"], timeout_seconds
    )
    server_check, list_output = _run_ollama_diagnostic_command(
        [resolved_command, "list"], timeout_seconds
    )
    listed_models = _ollama_listed_models(list_output) if server_check["status"] == "PASS" else set()
    model_available = model in listed_models

    inference_command = build_ollama_command(
        resolved_command,
        model,
        LOCAL_VERIFY_PROMPT,
        think_enabled=think_enabled,
    )
    redacted_command = [*inference_command[:-1], "<fixed-verification-prompt>"]
    if cli_check["status"] == "PASS" and server_check["status"] == "PASS" and model_available:
        inference_check, inference_output = _run_ollama_diagnostic_command(
            inference_command,
            timeout_seconds,
            evidence_command=redacted_command,
        )
        response_verified = "LOCAL_OLLAMA_OK" in inference_output
        if inference_check["status"] == "PASS" and not response_verified:
            inference_check["status"] = "FAIL"
            inference_check["error"] = "verification_marker_missing"
        inference_check["response_verified"] = response_verified
        inference_check["output_sha256"] = hash_text(inference_output) if inference_output else None
        inference_check["output_length"] = len(inference_output)
    else:
        inference_check = {
            ...
            "timed_out": False,
            "elapsed_seconds": 0.0,
            "error": "prerequisite_check_failed",
    ...
    status = "PASS"
    if any(check["status"] != "PASS" for check in (cli_check, server_check, inference_check)):
        status = "BLOCKED" if any(
            check["status"] == "BLOCKED" for check in (cli_check, server_check, inference_check)
        ) else "FAIL"
    ...
    return {
        ...
        "role": role,
        "model": model,
        "ollama_command": resolved_command,
        "started_at": started_at,
        "finished_at": utc_timestamp(),
        "elapsed_seconds": round(max(0.0, time.monotonic() - overall_start), 3),
        "model_available": model_available,
        "checks": {
            "cli": cli_check,
            "server_and_models": server_check,
            "inference": inference_check,
        },
    }


def _model_uses_full_gpu(ollama_ps_output: str, model: str) -> bool:
    """Return true only for an exact Ollama model row reporting full GPU use."""
    for line in (ollama_ps_output or "").splitlines():
        columns = line.split()
        if columns and columns[0] == model and "100% GPU" in line.upper():
            return True
    return False


def verify_local_cuda(
    ollama_command: str,
    model: str,
    timeout_seconds: int,
    *,
    ...
    """Prove an already-run local inference is resident on CUDA, if available.

    ``verify_local_ollama`` launches the selected model first. Ollama keeps a
    successful model resident briefly, so querying immediately afterwards gives
    an observable, prompt-safe GPU residency proof without a hosted fallback.
    """
    evidence = verify_local_ollama(
        ollama_command, model, timeout_seconds, role=role, think_enabled=think_enabled
    )
    resolved_command = evidence["ollama_command"]
    if evidence["status"] != "PASS":
        gpu_check = {
            "command": [resolved_command, "ps"],
            "status": "BLOCKED",
            "error": "local_inference_prerequisite_failed",
        }
        telemetry_check = {
            "command": ["nvidia-smi", "--query-gpu=index,name,memory.used,memory.total,utilization.gpu"],
            "status": "BLOCKED",
            "error": "local_inference_prerequisite_failed",
        }
    else:
        gpu_check, ps_output = _run_ollama_diagnostic_command(
            [resolved_command, "ps"], timeout_seconds
        )
        gpu_check["model_full_gpu"] = _model_uses_full_gpu(ps_output, model)
        if gpu_check["status"] == "PASS" and not gpu_check["model_full_gpu"]:
            gpu_check["status"] = "FAIL"
            gpu_check["error"] = "selected_model_not_100_percent_gpu"
        telemetry_command = [
            "nvidia-smi",
            "--query-gpu=index,name,memory.used,memory.total,utilization.gpu",
            "--format=csv,noheader,nounits",
        ]
        telemetry_check, telemetry_output = _run_ollama_diagnostic_command(
            telemetry_command, timeout_seconds
        )
        telemetry_check["telemetry_available"] = bool(telemetry_output.strip())
        if telemetry_check["status"] == "PASS" and not telemetry_check["telemetry_available"]:
            telemetry_check["status"] = "FAIL"
            telemetry_check["error"] = "nvidia_smi_telemetry_empty"

    checks = (evidence["checks"]["cli"], evidence["checks"]["server_and_models"],
              evidence["checks"]["inference"], gpu_check, telemetry_check)
    evidence["status"] = "PASS" if all(check["status"] == "PASS" for check in checks) else (
        "BLOCKED" if any(check["status"] == "BLOCKED" for check in checks) else "FAIL"
    )
    evidence["cuda_required"] = True
    evidence["checks"]["ollama_ps"] = gpu_check
    evidence["checks"]["nvidia_smi"] = telemetry_check
    return evidence
...
def run_builder_subprocess(
    ...
    try:
        ...
        return BuilderRunResult(
            ...
            started_at=started_at,
            finished_at=finished.strftime("%Y-%m-%dT%H:%M:%SZ"),
            error=f"Local agent command not found: {DEFAULT_LOCAL_AGENT_PATH}",
        )
    except subprocess.TimeoutExpired as exc:
        finished = datetime.now(timezone.utc)
        stdout = exc.output if isinstance(exc.output, str) else (exc.output or b"").decode("utf-8", "replace")
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--verify-local",
        action="store_true",
        help="Run explicit Ollama CLI/server/model checks plus one tiny local inference, print JSON, and exit.",
    )
    parser.add_argument(
        "--verify-local-cuda",
        action="store_true",
        help="Run local inference then require Ollama 100%% GPU residency and nvidia-smi telemetry, print JSON, and exit.",
    )
    parser.add_argument(
        "--verify-role",
        choices=("implementer", "validator"),
        default="implementer",
        help="Select the configured model used by --verify-local (default: implementer).",
    )
    parser.add_argument(
        "--verify-timeout",
        dest="verify_timeout_seconds",
        type=lambda value: normalize_timeout(int(value)),
        default=safe_env_int("TRUBA_OLLAMA_VERIFY_TIMEOUT_SECONDS", 120),
        help="Timeout in seconds for each --verify-local subprocess check.",
    )
    ...
    parser.add_argument(
        ...
        type=lambda value: normalize_timeout(int(value)),
        default=safe_env_int("TRUBA_BUILDER_TIMEOUT_SECONDS", 3600),
    )
    parser.add_argument(
        "--tester-timeout",
        dest="tester_timeout_seconds",
        type=lambda value: normalize_timeout(int(value)),
        default=safe_env_int("TRUBA_TESTER_TIMEOUT_SECONDS", 3600),
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.verify_local or args.verify_local_cuda:
        selected_model = args.builder_model if args.verify_role == "implementer" else args.tester_model
        verifier = verify_local_cuda if args.verify_local_cuda else verify_local_ollama
        evidence = verifier(
            args.ollama_command,
            selected_model,
            args.verify_timeout_seconds,
            role=args.verify_role,
